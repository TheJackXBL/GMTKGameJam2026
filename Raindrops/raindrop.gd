...
@onready var raindrop_sprite = $Sprite2D
...
@export var adhesion_force := 10.0 # How sticky raindrops are in general
...
@export var streak_alpha := 0.25

#Assigned by RaindropManager when spawned
var streak_container: Node2D
...
func create_streak() -> void:
	if streak_container == null:
		push_error("Raindrop has no streak_container assigned!")
		return
...
func add_streak_point() -> void:
	...
	streak.add_point(
		streak_container.to_local(global_position)
	)
	last_streak_position = global_position
...
func update_drop_size() -> void:
	...
	if collShape.shape is CircleShape2D:
		var circle_shape := collShape.shape as CircleShape2D
